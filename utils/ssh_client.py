...
class SSHClient(object):
    ...
    def load_system_known_hosts(self, path=None):
        host_keys_path = os.path.expanduser('~/.ssh/known_hosts') if path is None else path
        self.client.load_host_keys(host_keys_path)
    ...
    def _connect_with_key(self, **kwargs):
        if self.key is None:
            message = 'Private key is not specified for {}@{}'.format(self.username, self.host)
            logger.error(message)
            raise SSHConnectionFailed(message)
        self.client.connect(
            self.host, username=self.username, key_filename=self.key, passphrase=self.passphrase, **kwargs)
    ...
    def execute(self, command, raise_on_error=False, **kwargs):
        ...
        logger.info('Command executed in {} sec., return code: {}'
                    '\nStdOut:\n{}\nStdErr:\n{}'.format(r_code, exec_time, stdout, stderr))
